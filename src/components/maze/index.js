...
import { CANVAS_SIZE, SCALE, SPEED, DIRECTIONS, COLOR_NAMES } from "./constants"
...
const getNeighbors = (x, y) => DIRECTIONS.map(n => [n[0] + x, n[1] + y])
...
const dfs = (x, y) => {
  ...
  while (!stack.isEmpty()) {
    ...
    visit(node[0], node[1])
    const neighbors = getNeighbors(node[0], node[1]).filter(
      n => isInRange(n[0], n[1]) && !isVisited(n[0], n[1])
    )
    shuffle(neighbors)
    neighbors.forEach(n => {
      stack.push(n)
...
export const Maze = () => {
  ...
  const color = "light"
  const designs = ["maze", "dots"]
  const design = type === "mixed" ? designs[getRandomArbitrary(0, 2)] : type
  const circleSize = type === "mixed" ? sizes[getRandomArbitrary(0, 3)] : size
